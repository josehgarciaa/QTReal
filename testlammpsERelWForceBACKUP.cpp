...
void trialcallback(void *ptr, LAMMPS_NS::bigint ntimestep, int nlocal, int *id, double **x, double **f){
    ...
    if(info ->me == 0 && ntimestep==0){
        ...
        info -> kpm -> PeTm1 = etotal; //Store the potential energy //This is removed for nsteps>0
        ...
        std::cout<<"The norm of the randomphase wavepacket is "<<std::sqrt(Eta/(scalar)info -> kpm -> bsize)<<std::endl;
        DynEnergy = (emax/chebyshev::CUTOFF)*(Float) info -> kpm -> SiteList.size() * DynEnergy/(Float)info -> kpm -> bsize/std::sqrt(Eta/(scalar)info -> kpm -> bsize).real();
        ...
        EqEnergy = (emax/chebyshev::CUTOFF)*(Float) info -> kpm -> SiteList.size() * EqEnergy/(Float)info -> kpm -> bsize/std::sqrt(Eta/(scalar)info -> kpm -> bsize).real();
        ...
        if(!(std::fabs(etotal - info -> kpm -> PeTm1) <= std::fabs(EqEnergy.real() - DynEnergy.real())))
            std::cout<<"DEBUG: Potential Energy Change greater than Electronic energy change"<<std::endl;
        
        //if((EqEnergy.real() - DynEnergy.real() <0.) && (-(potential - info -> kpm -> PeTm1) <= 0.   ) && (std::fabs(potential - info -> kpm -> PeTm1) <= std::fabs(EqEnergy.real() - DynEnergy.real()))){
        if((-std::fabs(EqEnergy.real() - DynEnergy.real()) <0.) && (-(etotal - info -> kpm -> PeTm1) <= 0.   ) && (std::fabs(etotal - info -> kpm -> PeTm1) <= std::fabs(EqEnergy.real() - DynEnergy.real()))){
            //Eta = -(potential - info -> kpm -> PeTm1)/(EqEnergy.real() - DynEnergy.real());
            Eta = -(etotal - info -> kpm -> PeTm1)/-std::fabs((EqEnergy.real() - DynEnergy.real()));
        }
        else
            Eta=0.0;
        alpha = 1.0 - Eta;
        ...
        //if((EqEnergy.real() - DynEnergy.real() <0.) && (-(potential - info -> kpm -> PeTm1) <= 0.   ) && (std::fabs(potential - info -> kpm -> PeTm1) <= std::fabs(EqEnergy.real() - DynEnergy.real()))){
        if((EqEnergy.real() - DynEnergy.real() <0.) && (-(etotal - info -> kpm -> PeTm1) <= 0.   ) && (std::fabs(etotal - info -> kpm -> PeTm1) <= std::fabs(EqEnergy.real() - DynEnergy.real()))){
            //Eta = -(potential - info -> kpm -> PeTm1)/(EqEnergy.real() - DynEnergy.real());
            Eta = -(etotal - info -> kpm -> PeTm1)/-std::fabs((EqEnergy.real() - DynEnergy.real()));
        }
        else
        ...
        info -> kpm -> PeTm1 = etotal; //Store the potential energy
        //End Relaxation factor calculation  
