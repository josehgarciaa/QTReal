...
AuxMemory::AuxMemory(MPI_Comm comm)
{
  error = new AuxError(comm);
}
...
void *AuxMemory::smalloc(int n, const char *name)
{
  if (n == 0) return NULL;
  void *ptr = malloc(n);
  if (ptr == NULL) {
...
void *AuxMemory::srealloc(void *ptr, int n, const char *name)
{
  if (n == 0) {
    sfree(ptr);
    return NULL;
...
double **AuxMemory::create_2d_double_array(int n1, int n2, const char *name)
...
double **AuxMemory::grow_2d_double_array(double **array,
...
void AuxMemory::destroy_2d_double_array(double **array)
