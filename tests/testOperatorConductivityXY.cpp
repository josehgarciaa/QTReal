...
int main(Integer argc, char **argv){
    ...
    std::string name = "./OpDOS.dat";

    omp_set_num_threads(threads);
    //Unitary testlat("./pybuild","squaremine");
    //Unitary testlat("./pybuild","GrapheneSpinful");
    //Unitary testlat("./pybuild","TriangularLatticeP6mmm");
    //Unitary testlat("./pybuild","HaldaneModel");
    //Unitary testlat("./pybuild","MoS2");
    Unitary testlat("./pybuild","MoS2pahsescheck");
    testlat.RegisterOrbitals();
    testlat.RegisterHoppings();
    testlat.SetDeltas();
    const scalar emax = 6.1;
    PeriodicHamiltonian HamBuilder(testlat, nv1, nv2);
    std::vector<Integer> rows, cols, Orows, Ocols, Vrows, Vcols, Oprows, Opcols;
    std::vector<scalar> vals, Ovals, VXvals,VYvals, Opvals;
    ...
    SparseMat Hoppings, Onsites, VX, VY, Sz;
    ...
    chebyshev::CorrelationExpansionMoments(Sz,VY,VX,chebmom,gen,R/bsize);
    chebmom.ApplyJacksonKernel(0.0000001,0.0000001);
    Integer numdiv = 60*chebmom.HighestMomentNumber();
    std::ofstream cond("./OpConductivityXY.dat");
