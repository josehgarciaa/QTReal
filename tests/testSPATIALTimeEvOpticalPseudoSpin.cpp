...
int main(Integer argc, char **argv){
    ...
    GenerateFinitePseudoSpin(Oprows, Opcols, Opvals,grid);
    ...
    Op.SetLabel("PseudoSpin");
    Op.SetDimensions(SiteList.size(), SiteList.size());
    Op.SetNelem(Oprows.size());
    Op.ConvertFromCOO(Oprows, Opcols, Opvals);
        ...
        while(chebMom.CurrentTimeStep() != chebMom.MaxTimeStep()){
            ...
            if(counter == 0 || counter == evoxmeasure){
                if(chebMom.BlockSize()==1)
                    chebMom.SetInitVectors(Op,Prepr.OPV());
                else
                    chebMom.SetInitBlock(Op,Prepr.OPV());
                ...
                if(chebMom.BlockSize()==1)
                    Reference.SetInitVectors(Op,gen.Out());
                else
                    Reference.SetInitBlock(Op,gen.Out());
                for(size_t m=0; m<M; m++){
                    double scal = 2.0/(gen.NumberOfStates()*gen.BlockSize());
                    if(m==0)
                        scal*=0.5;
    ...
    std::string filename = "./NEQPseudoSpinOptical"+Hoppings.GetLabel()+"Nv1"+std::to_string(nv1)+"xNv2"+std::to_string(nv2)+"M"+std::to_string(M);
    ...
    filename = "./ReferenceNEQPseudoSpinOptical"+Hoppings.GetLabel()+"Nv1"+std::to_string(nv1)+"xNv2"+std::to_string(nv2)+"M"+std::to_string(M);
